...
static HeuristicsResult update_heuristics(oop o, bool allow_rebias)
{
    ...
    if ((revocation_count >= BiasedLockingBulkRebiasThreshold) && (revocation_count < BiasedLockingBulkRevokeThreshold) && (last_bulk_revocation_time != 0) && (cur_time - last_bulk_revocation_time >= BiasedLockingDecayTime)) {
    ...
    JavaThread* requesting_thread)
{
    ...
    if (bulk_rebias) {
        // 批量重偏向模式，kclass的Epoch自增1，所有存活线程持有的属于该kclass的Java锁对象的Mark Word中的Epoch同自增1，
        // 余下的属于该kclass的Java锁对象的Mark Word中的Epoch不自增1，即过期
        ...
        // kclass的Mark Word设为无锁状态，即关闭该kclass的偏向锁
        // 类的偏向锁标志位设为关闭，属于该类的Java锁对象尝试申请锁时，不会再进入偏向锁状态；属于该类新分配的Java锁对象的mark word为无锁状态
    ...
    assert(!o->mark()->has_bias_pattern() || (attempt_rebias_of_object && (o->mark()->biased_locker() == requesting_thread)),
...
BiasedLocking::Condition BiasedLocking::revoke_and_rebias(Handle obj, bool attempt_rebias, TRAPS)
{
    ...
    //对应的是Java锁对象的Mark Word
    markOop mark = obj->mark();

    if (mark->is_biased_anonymously() && !attempt_rebias) {
        // 匿名偏向+attempt_rebias=false，表示是计算HashCode触发的流程
