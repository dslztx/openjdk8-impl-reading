...
void ObjectSynchronizer::fast_enter(Handle obj, BasicLock* lock, bool attempt_rebias, TRAPS)
{
    if (UseBiasedLocking) {
        if (!SafepointSynchronize::is_at_safepoint()) {
            ...
            // VM线程执行到这里，attempt_rebias必为false
    ...
    // fast_enter跟slow_enter方法相比就是再度尝试申请偏向锁，如果成功直接返回
...
void ObjectSynchronizer::slow_enter(Handle obj, BasicLock* lock, TRAPS)
{
    ...
    if (mark->is_neutral()) {
        ...
        // 第一次申请轻量级锁，把displaced_header设置好
        ...
        // 轻量级锁锁重入，那么displaced_header只需要为空就可以
  ...
  if (mark->has_monitor() && mark->monitor()->is_entered(THREAD)) {
    ...
    // 申请重量级锁，displaced_header设置成对应于重量级锁的一个特殊值
    lock->set_displaced_header(markOopDesc::unused_mark());

    // 申请重量级锁对象，即ObjectMonitor对象，然后调用enter方法尝试去获得该重量级锁
...
intptr_t ObjectSynchronizer::FastHashCode(Thread* Self, oop obj)
{
    if (UseBiasedLocking) {
        ...
        if (obj->mark()->has_bias_pattern()) {
            ...
            assert(Universe::verify_in_progress() || !SafepointSynchronize::is_at_safepoint(),
    ...
    assert(Universe::verify_in_progress() || Self->is_Java_thread(), "invariant");
    assert(Universe::verify_in_progress() || ((JavaThread*)Self)->thread_state() != _thread_blocked, "invariant");
...
// ower_other.
ObjectSynchronizer::LockOwnership ObjectSynchronizer::query_lock_ownership(JavaThread* self, Handle h_obj)
{
...
ObjectMonitor* ATTR ObjectSynchronizer::inflate(Thread* Self, oop object)
{
    ...
    for (;;) {
        ...
        // *  Inflated     - just return，即重量级锁状态
        // *  Stack-locked - coerce it to inflated，即轻量级锁状态
        // *  INFLATING    - busy wait for conversion to complete，一个额外的中间状态
        // *  Neutral      - aggressively inflate the object，即无锁状态
        // *  BIASED       - Illegal.  We should never see this，即偏向锁状态，这里不可能再遇到偏向锁状态

        // CASE: inflated
        if (mark->has_monitor()) {
            //已经是重量级锁状态，直接返回该重量级锁对象
        ...
        if (mark->has_locker()) {
            ...
            m->Recycle();
            m->_Responsible = NULL;
            //OwnerIsThread=0表示_owner字段的值是Lock Record的指针
            m->OwnerIsThread = 0;
            m->_recursions = 0;
            ...
            //mark->locker返回的是一个Lock Record的指针
    ...
    ObjectMonitor** FreeHeadp, ObjectMonitor** FreeTailp)
{
    ...
    if (mid->is_busy()) {
        if (ClearResponsibleAtSTW)
            mid->_Responsible = NULL;
        deflated = false;
    } else {
...
void ObjectSynchronizer::deflate_idle_monitors()
{
    ...
    if (ObjectMonitor::_sync_Deflations != NULL)
        ObjectMonitor::_sync_Deflations->inc(nScavenged);
    if (ObjectMonitor::_sync_MonExtant != NULL)
        ObjectMonitor::_sync_MonExtant->set_value(nInCirculation);
...
public:
    ReleaseJavaMonitorsClosure(Thread* thread)
        : THREAD(thread)
    {
    }
    ...
    int* warning_cnt_ptr)
{
    ...
    if (verbose)
        tty->print_cr("INFO: offset(stwRandom)=%u", offset_stwRandom);
