...
#define DO_UPDATE_INSTRUCTION_COUNT(opcode)                                            \
    {                                                                                  \
        ...
        if (StopInterpreterAt && StopInterpreterAt == BytecodeCounter::_counter_value) \
            os::breakpoint();                                                          \
        if (TraceBytecodes) {                                                          \
    ...
    {                                                                                                      \
        if (_jvmti_interp_events) {                                                                        \
            if (JvmtiExport::should_post_single_step()) {                                                  \
                ...
                if (THREAD->jvmti_thread_state() && THREAD->jvmti_thread_state()->is_earlyret_pending()) { \
 ...
 */
#if defined(VM_JVMTI)
void BytecodeInterpreter::runWithChecks(interpreterState istate)
{
#else
void BytecodeInterpreter::run(interpreterState istate)
{
    ...
    if (istate->_msg != initialize) {
        ...
        if (EnableInvokeDynamic || FLAG_IS_CMDLINE(EnableInvokeDynamic)) {
            ...
            assert(labs(istate->_stack_base - istate->_stack_limit) == (istate->_method->max_stack() + extra_stack_entries + 1), "bad stack limit");
        }
#ifndef SHARK
...
// #define HACK
    ...
    guarantee(istate->msg() == initialize || topOfStack >= istate->stack_limit() && topOfStack < istate->stack_base(),
    ...
    case initialize: {
        if (initialized++)
            ShouldNotReachHere(); // Only one initialize call.
...
#define OPC_DSTORE_n(num)                           \
    ...
    UPDATE_PC_AND_TOS_AND_CONTINUE(1, -2);
            ...
            CASE(_pop2)
                : /* Discard the top 2 items on the stack */
                UPDATE_PC_AND_TOS_AND_CONTINUE(1, -2);
...
#define COMPARISON_OP(name, comparison)                            \
    ...
    {                                                              \
        ...
        int skip = cmp                                             \
            ? (int16_t)Bytes::get_Java_u2(pc + 1)                  \
            : 3;                                                   \
...
{
    switch (msg) {
    case BytecodeInterpreter::no_request:
        return ("no_request");
    case BytecodeInterpreter::initialize:
        return ("initialize");
    // status message to C++ interpreter
    case BytecodeInterpreter::method_entry:
        return ("method_entry");
    case BytecodeInterpreter::method_resume:
        return ("method_resume");
    case BytecodeInterpreter::got_monitors:
        return ("got_monitors");
    case BytecodeInterpreter::rethrow_exception:
        return ("rethrow_exception");
    // requests to frame manager from C++ interpreter
    case BytecodeInterpreter::call_method:
        return ("call_method");
    case BytecodeInterpreter::return_from_method:
        return ("return_from_method");
    case BytecodeInterpreter::more_monitors:
        return ("more_monitors");
    case BytecodeInterpreter::throwing_exception:
        return ("throwing_exception");
    case BytecodeInterpreter::popping_frame:
        return ("popping_frame");
    case BytecodeInterpreter::do_osr:
        return ("do_osr");
    // deopt
    case BytecodeInterpreter::deopt_resume:
        return ("deopt_resume");
    case BytecodeInterpreter::deopt_resume2:
        return ("deopt_resume2");
    default:
        return ("BAD MSG");
    }
}
void BytecodeInterpreter::print()
{
